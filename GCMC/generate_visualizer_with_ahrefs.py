#!/usr/bin/env python3
"""Ahrefsアップロード機能付きHTMLビジュアライザーを生成"""
import json

# JSONデータを読み込み
with open('gcmc_link_analysis_report.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

# HTMLテンプレート（Ahrefsアップロード機能付き）
html_template = '''<!DOCTYPE html>
<html lang="ja">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>GCMC 内部リンク構造 & 広告配置分析</title>
    ...
    <script>
        ...
        function showDetails(node) {
            ...
            const internalLinks = node.internal_links.slice(0, 20).map(l => {
                ...
                const displayText = linkedPage ? (linkedPage.title || l.split('/').pop()) : l.split('/').pop();
            ...
            document.querySelector('.detail-panel').innerHTML = `
                ...
                <div style="margin: 15px 0;"><a href="${node.url}" target="_blank" style="color: #667eea; text-decoration: none;">${node.title}</a></div>
                <div><h3 style="color: #667eea;">📊 基本情報</h3>
                <ul class="link-list">
                ...
                ${keywords.length > 0 ? `<div><h3 style="color: #667eea;">🎯 上位キーワード (20位以内)</h3><div style="margin-top: 10px;">${keywordsHTML}</div></div>` : ''}
                ${adLinks ? `<div><h3 style="color: #667eea;">💰 広告リンク (${node.ad_links.length})</h3><ul class="link-list">${adLinks}</ul></div>` : ''}
                ${internalLinks ? `<div><h3 style="color: #667eea;">🔗 内部リンク (${node.internal_links.length})</h3><ul class="link-list">${internalLinks}</ul></div>` : ''}
            `;
        }
        ...
        function setupEventListeners() {
            ...
            document.getElementById('searchInput').addEventListener('input', e => {
                ...
                filteredData = term === '' ? allData : {...allData, pages: allData.pages.filter(p => 
                    p.url.toLowerCase().includes(term) || p.title.toLowerCase().includes(term) || (p.keyword && p.keyword.toLowerCase().includes(term))
                )};
                renderGraph();
            });
        }
...
# HTMLファイルを書き込み
with open('gcmc_link_visualizer_with_ahrefs.html', 'w', encoding='utf-8') as f:
    f.write(html_template)

print("✓ Ahrefsアップロード機能付きHTMLファイルを生成しました: gcmc_link_visualizer_with_ahrefs.html")
