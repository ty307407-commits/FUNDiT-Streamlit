# QBC Link Visualizer with Ahrefs Integration

QBCドメインの内部リンク構造と広告配置を可視化し、Ahrefsのキーワードデータと統合するツールです。
...
このスクリプトは`qb_clinic_urls_with_h1_keywords.csv`を読み込み、`qbc_link_analysis_report.json`を生成します。
...
スタンドアロンHTML（`qbc_link_visualizer_with_ahrefs.html`）が生成されます。

### 3. ブラウザで開く

生成された`qbc_link_visualizer_with_ahrefs.html`をダブルクリック、またはブラウザにドラッグ＆ドロップします。
...
### 入力データ (qb_clinic_urls_with_h1_keywords.csv)
...
```
QBC/
├── qbc_link_analyzer.py                   # リンク分析スクリプト
├── generate_visualizer_with_ahrefs.py     # HTML生成スクリプト
├── qb_clinic_urls_with_h1_keywords.csv    # 入力データ
├── qbc_link_analysis_report.json          # 分析結果
└── qbc_link_visualizer_with_ahrefs.html   # 最終成果物（スタンドアロン）
```
