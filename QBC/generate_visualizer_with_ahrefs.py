#!/usr/bin/env python3
"""QBC用 - Ahrefsアップロード機能付きHTMLビジュアライザーを生成"""
import json
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .rank-1-10 { border-left-color: #4CAF50; background: #e8f5e9; }
        .rank-11-20 { border-left-color: #FF9800; background: #fff3e0; }
        .stats {
            padding: 20px 30px;
            display: grid;
        ...
        .stat-card.monetization { background: linear-gradient(135deg, #ffa8c5 0%, #ffd3e0 100%); }
        .stat-card.feeder { background: linear-gradient(135deg, #ffc0d3 0%, #ffe5ee 100%); }
        .stat-card.hybrid { background: linear-gradient(135deg, #fff0d4 0%, #ffd1a8 100%); }
        ...
        .badge.monetization { background: #ffa8c5; color: #831843; }
        .badge.feeder { background: #ffc0d3; color: #b91c50; }
        .badge.hybrid { background: #fff0d4; color: #92400e; }
...
<body>
    <div class="container">
        <header>
            ...
            <p class="subtitle">収益化ページとフィーダーページの可視化 + Ahrefsデータ統合</p>
        </header>
        <div class="controls">
            ...
            <div class="control-group">
                <input type="file" id="ahrefsFile" accept=".csv">
                <button class="upload-btn" onclick="document.getElementById('ahrefsFile').click()">
                    📊 Ahrefsデータをアップロード
                </button>
                <span id="uploadStatus" style="color: #28a745; font-weight: 600;"></span>
            </div>
        </div>
            ...
            <div id="network-graph">
                <div class="legend">
                    ...
                    <div class="legend-item">
                        <div class="legend-circle" style="background: #ffa8c5;"></div>
                        <span>収益化ページ</span>
                    </div>
                    <div class="legend-item">
                        <div class="legend-circle" style="background: #ffc0d3;"></div>
                        <span>フィーダーページ</span>
                    </div>
                    <div class="legend-item">
                        <div class="legend-circle" style="background: #fff0d4;"></div>
                        <span>ハイブリッド</span>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        // Ahrefsファイルアップロード処理
        document.getElementById('ahrefsFile').addEventListener('change', (e) => {
            ...
            reader.onload = (event) => {
                ...
                parseAhrefsCSV(csv);
                document.getElementById('uploadStatus').textContent = '✓ データ読み込み完了';
                setTimeout(() => document.getElementById('uploadStatus').textContent = '', 3000);
            };
            reader.readAsText(file);
        });

        function parseAhrefsCSV(csv) {
            const lines = csv.split('\\n');
            const headers = lines[0].split(',');
            
            // ヘッダーのインデックスを取得
            const keywordIdx = headers.indexOf('Keyword');
            const urlIdx = headers.indexOf('Current URL');
            const positionIdx = headers.indexOf('Current position');
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                const volume = parseInt(values[volumeIdx]) || 0;
            ...
            // ボリューム順にソート
            Object.keys(ahrefsData).forEach(url => {
                ahrefsData[url].sort((a, b) => b.volume - a.volume);
            });
        }
        ...
        function renderStats() {
            ...
            document.getElementById('stats').innerHTML = `
                ...
                <div class="stat-card" style="background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); color: white;"><div class="stat-value">${s.avg_internal_links}</div><div class="stat-label">平均内部リンク数</div></div>
            `;
        }
        ...
        function renderGraph() {
            ...
            const color = d3.scaleOrdinal().domain(['monetization', 'feeder', 'hybrid']).range(['#ffa8c5', '#ffc0d3', '#fff0d4']);
        ...
        function showDetails(node) {
            ...
            // 内部リンクをページタイトル表示+クリック可能に (QBC用: h1を使用)
            const internalLinks = node.internal_links.slice(0, 20).map(l => {
                ...
                return `<li><a href="${l}" target="_blank" style="color: #f093fb; text-decoration: none;">${displayText}</a></li>`;
            }).join('');
            
            const adLinks = node.ad_links.slice(0, 10).map(a => `<li class="ad-link">${a.text || 'リンク'}<span class="ad-type">${a.type}</span></li>`).join('');
            
            // Ahrefsデータから20位以内のキーワードを取得
            const keywords = ahrefsData[node.url] || [];
            const keywordsHTML = keywords.length > 0 ? keywords.map(kw => {
                const rankClass = kw.position <= 10 ? 'rank-1-10' : 'rank-11-20';
                return `<span class="keyword-badge ${rankClass}"><strong>${kw.position}位</strong> ${kw.keyword} (${kw.volume})</span>`;
            }).join('') : '<p style="color: #999;">Ahrefsデータなし</p>';
            ...
            document.querySelector('.detail-panel').innerHTML = `
                ...
                ${keywords.length > 0 ? `<div><h3 style="color: #f093fb;">🎯 上位キーワード (20位以内)</h3><div style="margin-top: 10px;">${keywordsHTML}</div></div>` : ''}
...
# HTMLファイルを書き込み
with open('qbc_link_visualizer_with_ahrefs.html', 'w', encoding='utf-8') as f:
    f.write(html_template)

print("✓ QBC用 Ahrefsアップロード機能付きHTMLファイルを生成しました: qbc_link_visualizer_with_ahrefs.html")
